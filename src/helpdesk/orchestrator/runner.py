...
resolution_attempts 由本层计数(RESOLVE 分支 +1)。"""
...
from helpdesk.orchestrator.nodes.intake import run_intake
...
# 节点注册表:clarify / resolve / escalate / close 等按计划 D3–D4 注册。
...
def handle_message(
    ...
    for _ in range(_MAX_HOPS):
        ...
        if decision.branch is Branch.ESCALATE:
            state.escalation.required = True
            state.escalation.reason_code = decision.reason_code
