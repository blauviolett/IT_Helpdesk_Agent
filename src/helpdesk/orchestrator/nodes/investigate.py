...
def run_investigate(state: CaseState, ctx: Any) -> None:
    ...
    for batch_no in range(1, _MAX_BATCHES + 1):
        if not calls:
            break  # 终止 1:空工具列表
        if _budget_over(state.budget, limits):
            break  # 终止 3:预算触顶
        ctx.tracer.event(
            state.case_id,
            "investigate_batch",
...
def _budget_over(budget: Budget, limits: dict[str, Any]) -> bool:
    """与 decide E5 同口径(guide §4.1 L2)。"""
    return (
        budget.tool_calls >= limits["tool_calls_max"]
        or budget.turns >= limits["turns_max"]
        or budget.llm_cost_usd >= limits["llm_cost_usd_max"]
        or budget.elapsed_sec >= limits["elapsed_sec_max"]
    )
